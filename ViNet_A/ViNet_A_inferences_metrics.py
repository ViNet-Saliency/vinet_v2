...
parser.add_argument('--model_save_root_path',default="~/ICASSP_Saliency/saved_models/", type=str)
...
# model_tag = '%s_%s_%s_%s_%s_%s_%sbs_seed%s.pt'%(args.model_tag,args.dataset,args.split,args.neck_name,args.decoder_groups,'kldiv'*args.kldiv + 'cc'*args.cc + 'nss'*args.nss +'sim'*args.sim,args.batch_size,seed)
...
def test(model, loader, device, args):
	...
	for idx, sample in tqdm(enumerate(loader)):
		...
		if args.save_inferences and args.save_path is not None:
			os.makedirs(join(args.save_path, 'inferences','ViNet_A',video_name),exist_ok=True)
			img_save(pred_sal, join(args.save_path,'inferences','ViNet_A', video_name, 'img_%05d.png'%(mid_frame+1)), normalize=True)
			continue
...
with open(join(os.path.expanduser(args.metrics_save_path),args.dataset, model_name + '_split' + str(args.split) + '_' + 'video_metrics_dict.json'), 'w') as f:
	f.write(r)
...
# # parser.add_argument('--train_path_data',default="/ssd_scratch/cvit/jainsiddharth/datasets/DHF1K/annotation", type=str)
# # parser.add_argument('--val_path_data',default="/ssd_scratch/cvit/jainsiddharth/datasets/DHF1K/val", type=str)
...
# 	print("Loading pretrained EEAA model weights if any: ",args.checkpoint_path)
...
# 		pred_sal = cv2.resize(pred_sal, (gt_sal.shape[1], gt_sal.shape[0]))
# 		# pred_sal = blur(pred_sal).unsqueeze(0).cuda()
# 		pred_sal = torch.FloatTensor(pred_sal).unsqueeze(0).cuda()
...
# 		if args.save_inferences and args.save_path is not None:
# 			os.makedirs(join(args.save_path, 'inferences','ViNet-A',video_name),exist_ok=True)
# 			img_save(pred_sal, join(args.save_path,'inferences','ViNet-A', video_name, 'img_%05d.png'%(mid_frame+1)), normalize=True)
# 			continue
