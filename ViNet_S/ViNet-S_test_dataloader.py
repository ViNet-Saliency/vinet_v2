...
class SoundDatasetLoader(Dataset):
	def __init__(self, len_snippet, dataset_name='DIEM', split=1, mode='train', use_sound=False, use_vox=False,args=None):
		''' mode: train, val, save '''
		path_data = '/home/sid/Audio-Visual-SaliencyDatasets'
		...
		if self.mode=='train':
			...
			# self.list_indata = self.list_indata[:1]
			for v in self.video_names:
				frames = os.listdir(join(path_data, 'annotations', dataset_name, v, 'maps'))
				frames.sort()
				num_frames = len(frames)
				# print("Video: ", v, "Frames: ", num_frames)
...
class DHF1KDataset(Dataset):
	def __init__(self, path_data, len_snippet, mode="train", multi_frame=0, alternate=1, frames_path='images',args=None):
		''' mode: train, val, save '''
		self.path_data = path_data
		...
		if self.mode == "train":
			...
			for v in self.video_names:
				num_frames = len(os.listdir(os.path.join(path_data,v,frames_path)))
				for i in range(0,num_frames):
					self.list_num_frame.append((v,i))
...
class Hollywood_UCFDataset(Dataset):
	def __init__(self, path_data, len_snippet, mode="train", frame_no="last", multi_frame=0, frames_path='images',args=None):
	...
	def __getitem__(self, idx):
		...
		# added
			...
			# fix_map_filename = list_sal_clips[end_idx+1].split('/')[-1].split('.')[0]
			# video_clip_name = list_sal_clips[end_idx+1].split('/')[-3]


			fix = sio.loadmat(join(self.path_data,file_name,'fixation','maps','{}.mat'.format(list_sal_clips[end_idx].split('.')[0])))
...
class Hollywood_ViNet_Dataset(Dataset):
	def __init__(self, path_data, len_snippet, mode="train", frame_no="last", multi_frame=0, frames_path='images'):
		''' mode: train, val, perframe 
			frame_no: last, middle
		'''
		self.path_data = path_data
		...
		# added
			...
			# print('video_clips: ', self.video_clips)
...
class mvva_vinet_dataset(Dataset):
	def __init__(self, len_snippet, dataset_name='mvva', split=1, mode='train',use_skip=True):
		...
		self.fixation_data_path = '/home/sid/mvva_dataset/fixation_data_mvva'
		self.gt_sal_maps_path = '/home/sid/mvva_dataset/annotations/mvva'
		self.videos_frames_root_path = '/home/sid/mvva_dataset/video_frames/mvva'
		...
		self.list_indata = []
		with open(join('/home/sid/mvva_dataset/fold_lists', file_name), 'r') as f:
			for line in f.readlines():
				name = line.split(' ')[0].strip()
