...
class SoundDatasetLoader(Dataset):
    def __init__(self, len_snippet, dataset_name='DIEM', split=1, mode='train', use_sound=False, use_vox=False):
        ...
        if self.mode=='train':
            # 
            if self.dataset_name == 'ETMD_av':
                for video_name in self.list_indata:
                    num_frames = len(os.listdir(os.path.join(path_data,'annotations', dataset_name, v, 'maps')))
                    scene_start = 0
                    scene_end = num_frames // 3
                    self.list_num_frame.append((video_name,scene_start,scene_end))
                    scene_start = num_frames // 3
                    scene_end = (2*num_frames) // 3
                    self.list_num_frame.append((video_name,scene_start,scene_end))
                    scene_start = (2*num_frames) // 3
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            if self.dataset_name != 'ETMD_av':
                ...
                video_name,scene_start,scene_end = self.list_num_frame[idx]
                while 1:
                    start_idx = np.random.randint(scene_start,scene_end)
                    if self.check_frame(join(self.path_data, 'annotations', self.dataset_name, video_name, 'maps', 'eyeMap_%05d.jpg'%(start_idx+self.len_snippet))):
                        break
                    else:
                        print("No saliency defined in train dataset")
        ...
        if self.mode != 'test':
            ...
            binary_img = np.array(loadmat(join(self.path_data, 'annotations', self.dataset, video_name, 'fixMap_%05d.mat')%(end_idx+1))['eyeMap'])
